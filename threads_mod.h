...
//
// Modified by Strickland to make Windows and pthreads versions
// to return same thing.
//
...
static int fl_create_thread(Fl_Thread& t, void *(*f) (void *), void* p) {
  ...
  // _beginthread returns -1L on error and the thread number on success 
  // pthread_create returns 0 on success and EAGAIN on error.
  return (t == -1L) ? EAGAIN : 0;
}
...
static int fl_create_thread(Fl_Thread& t, void *(*f) (void *), void* p) {
  t = (Fl_Thread)_beginthread((void(* )( void * ))f, 32000, p);
  // _beginthread returns -1L on error and the thread number on success 
  // pthread_create returns 0 on success and EAGAIN on error.
  // Force Windows to behave like pthreads.
  return (t == -1L) ? EAGAIN : 0;
}
